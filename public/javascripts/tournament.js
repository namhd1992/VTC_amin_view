...
var item_name = "Tournament";
var item_names = "Tournaments";
var deleted_id = null;
...
var load_page = function() {
    ...
    page_datatable = $('#page_datatable').DataTable({
        ...
        "columns": [
            ...
            },
            {
                ...
                render: function(data, type, full, meta) {
                    ...
                    var news_date = moment.unix(full.dateStart / 1000).format(format.value);
                    return news_date;
                }
            },
            {
                ...
                render: function(data, type, full, meta) {
                    var format = getSetting(_setting_data, "date_format");
                    var news_date = moment.unix(full.dateEnd / 1000).format(format.value);
                    return news_date;
                }
            },
            {
        ...
        "fnDrawCallback": function(oSettings) {
            ...
            $(".btn_edit_item").click(function() {
                ...
                $('#modal_add_edit .txt_add_edit_item_code').val(obj_temp.tournamentsCode);
                $("#single_select_game").val(obj_temp.inGame.id);
                load_autocomplete_component();
                $('.txt_add_edit_item_date_start').val(moment.unix(obj_temp.dateStart / 1000).format("YYYY-MMMM-D HH:mm"));
                $('.txt_add_edit_item_date_end').val(moment.unix(obj_temp.dateEnd / 1000).format("YYYY-MMMM-D HH:mm"));
    ...
    $('.btn_confirm_yes').click(function() {
        $.ajax({
            url: base_request_url + "/admin/tournament?tournamentId=" + deleted_id,
            type: "DELETE",
            contentType: "application/json",
            dataType: "json",
            beforeSend: function(xhr) {
    ...
    $('.btn_modal_save_new').click(function() {
        if ($(".txt_add_edit_item_name").val() == "") {
            show_toast_notif("error", "Unable to create tournament", "Tournament name is required", function() {});
            return;
        }
        if ($(".txt_add_edit_item_date_start").val() == "") {
            show_toast_notif("error", "Unable to create tournament", "Tournament start date is required", function() {});
            return;
        }
        if ($(".txt_add_edit_item_date_end").val() == "") {
            show_toast_notif("error", "Unable to create tournament", "Tournament end date is required", function() {});
            return;
        }
        ...
        if (start_date > end_date) {
            show_toast_notif("error", "Unable to create tournament", "Date range is invalid", function() {});
            return;
        }
        _send_info.data = {
            "tournamentName": $(".txt_add_edit_item_name").val(),
            "tournamentCode": $(".txt_add_edit_item_code").val(),
