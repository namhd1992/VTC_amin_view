...
var load_page = function () {
	...
	page_datatable = $('#page_datatable').DataTable({
		ajax: {
			"url": base_request_url + "/admin/category",
			"type": "GET",
			"data": function () {
				var d = {};
				d.limit = _per_page;
		...
		"fnDrawCallback": function (oSettings) {
			...
			$('.btn_delete_item').click(function () {
				...
				editing_obj = JSON.parse($(this).attr("item_obj"));
	...
	$(".btn_confirm_yes").click(function () {
		...
		$.ajax({
			url: base_request_url + "/admin/category?categoryId=" + deleted_id,
			type: "DELETE",
			...
			success: function () {
				...
				show_toast_notif("success", "Successfully", "Deleted: '" + editing_obj.name + "'");
			},
	...
	$('.btn_modal_save_new').click(function () {
		_send_info.data = {
			...
			"listTagId": $.merge(tag_array, package_array)
		};
		$.ajax({
			url: base_request_url + "/admin/category",
			type: "POST",
			...
			success: function (data) {
				...
				if (data.statusCode == "T") {
					show_toast_notif("success", "Successfully", "Created: '" + $(".txt_add_edit_channel_name").val() + "'");
					$('#modal_add_edit').modal('hide');
				} else {
	...
	$('.btn_modal_save_update').click(function () {
		...
		$.ajax({
			...
			success: function () {
				...
				_send_info.data = {};
				page_datatable.ajax.reload(null, false);
				show_toast_notif("success", "Successfully", "Updated '" + $(".txt_add_edit_channel_name").val() + "'");
			},
			error: function (error) {
				show_toast_notif("error", "Error", "'" + error.responseText + "'");
			},
	...
	$('.btn_modal_save_position').click(function () {
		...
		$.ajax({
			url: base_request_url + "/admin/positionCategory",
			type: "POST",
			...
			success: function (data) {
				...
				page_datatable.ajax.reload(null, false);
				if (data.statusCode == "T") {
					show_toast_notif("success", "Successfully", "Positions are updated");
				} else {
					show_toast_notif("error", "Error", "'" + data.onlyMessage + "'");
				}
