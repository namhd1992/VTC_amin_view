...
var load_page = function() {
    ...
    page_datatable = $('#page_datatable').DataTable({
        ...
        "columns": [
            ...
            { title: "Game", data: "inGame.name" },
            {
                ...
                render: function(data, type, full, meta) {
                    var str_data = "";
                    str_data += full.firstTeam.name + " - " + full.secondTeam.name;
                    return str_data;
                }
            },
            {
            ...
            },
            {
                ...
                render: function(data, type, full, meta) {
                    ...
                    var str_date = moment.unix(full.startTime / 1000).format(format.value);
                    return str_date;
                }
            },
            {
                ...
                render: function(data, type, full, meta) {
                    ...
                    return_str += "<a class='btn_event_link' item_id='" + full.id + "' href='/betting_events?match=" + full.id + "'><i class='icon-calendar'></i> Events </a>";
                    return return_str;
                }
            }
        ],
        ...
        "fnDrawCallback": function(oSettings) {
            ...
            $(".btn_edit_item").click(function() {
                ...
                $('#modal_add_edit .txt_add_edit_item_live').val(obj_temp.urlShowMatch);
                ...
                var format = getSetting(_setting_data, "date_format");
                $('.txt_add_edit_item_start_date').val(moment.unix(obj_temp.startTime / 1000).format(format.value));
                ...
                //set state
                $(".select_state option").filter(function() {
                    return $.trim($(this).val()) == obj_temp.state;
                }).prop('selected', true);
    ...
    $(".btn_add_new_item").click(function() {
        ...
        $('.select_status button').css('background-color', '#32c5d2');
        var format = getSetting(_setting_data, "date_format");
        $('.txt_add_edit_item_start_date').val(moment(new Date().getTime() + (24 * 60 * 60 * 1000)).format(format.value));
    ...
    $('.btn_modal_save_new').click(function() {
        ...
        var url_show_match = $(".txt_add_edit_item_live").val();
        if (url_show_match == "") {
            url_show_match = null;
        }
        _send_info.data = {
            "matchName": $(".txt_add_edit_item_name").val(),
            "urlShowMatch": url_show_match,
    ...
    $.ajax({
        ...
        success: function(data) {
            $(data.dataArr).each(function(key, item) {
                if (key == 0) {
                    $("#single_select_game").append("<option selected value='" + item.id + "'> " + item.name + "</option>");
                } else {
