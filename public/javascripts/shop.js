...
var load_page = function () {
	...
	page_datatable = $('#page_datatable').DataTable({
		...
		"fnDrawCallback": function (oSettings) {
			...
			$(".btn_edit_item").click(function () {
				...
				$('#modal_add_edit .txt_add_edit_item_big_image').val(obj_temp.bigImage);
				$('#modal_add_edit .txt_add_edit_item_url').val(obj_temp.redirectUrl);
				$('#single_select_game').val(obj_temp.gameId);
	...
	$('.btn_confirm_yes').click(function () {
		$.ajax({
			url: base_request_url + "/admin/shoping?id=" + deleted_id,
			type: "DELETE",
			contentType: "application/json",
			dataType: "json",
			beforeSend: function (xhr) {
	...
	$('.btn_modal_save_new').click(function () {
		...
		_send_info.data = {
			...
			"bigImage": $(".txt_add_edit_item_big_image").val(),
			"redirectUrl": $(".txt_add_edit_item_url").val(),
	...
	$.ajax({
		...
		success: function (data) {
			$(data.data).each(function (key, item) {
				$("#single_select_game").append("<option value='" + item.scoinGameId + "'> " + item.keyName + "</option>");
			});
			load_autocomplete_component();
		},
		error: function (err) {
