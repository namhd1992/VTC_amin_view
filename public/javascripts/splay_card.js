...
var item_name = "Splay Card";
var item_names = "Splay Cards";
var deleted_id = null;
var channel_arr = [];
...
var load_page = function () {
	...
	page_datatable = $('#page_datatable').DataTable({
		ajax: {
			"url": base_request_url + "/admin/splayCard",
			"type": "GET",
			"data": function (d) {
		...
		responsive: true,
		columns: [
			{ title: "ID", data: "missionId" },
			{ title: "Mission", data: "missionName" },
			{ title: "Tổng giftcode", data: "total" },
			{ title: "Đã Phát", data: "given" },
			{ title: "Còn Lại", data: "available" },
			{
				title: "Status",
				data: null,
			...
			{
				title: "Ngày hết hạn Giftcode",
				data: null,
				render: function (data, type, full, meta) {
					var return_str = "";
					return_str = moment.unix(parseInt(full.expirationDate) / 1000).format("YYYY-MM-D");
					return return_str;
				}
			}
		],
		"aoColumnDefs": [
			{ responsivePriority: 1, "sWidth": "10%", 'aTargets': [0] },
			{ responsivePriority: 2, "sWidth": "30%", 'aTargets': [1] },
	...
	$(".btn_add_new_item").click(function () {
		...
		$('.file_add_edit_item_giftcode').attr("name", "fileCard");
		channel_arr = [];
