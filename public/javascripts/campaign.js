...
var item_name = "Campaign";
var item_names = "Campaigns";
var deleted_id = null;
...
var load_page = function () {
	...
	page_datatable = $('#page_datatable').DataTable({
		"ajax": {
			"url": base_request_url + "/admin/campaign",
			"type": "GET",
			"data": function (d) {
		...
		"columns": [
			...
			},
			{
				...
				render: function (data, type, full, meta) {
					...
					return_str += "<a class='btn_view_code' item_id='" + full.id + "' item_obj='" + JSON.stringify(full) + "'><i class='icon-puzzle'></i> Code </a>";
					return return_str;
				}
			}
		],
		...
		"fnDrawCallback": function (oSettings) {
			...
			$(".btn_edit_item").click(function () {
				...
				item_count = obj_temp.splayBanners.length;
				$(".more_item").remove();
				$(".quantity_left").parent().remove();
				console.log(obj_temp.splayBanners);
				$(obj_temp.splayBanners).each(function (key, item) {
					if (key > 0) {
						var html_str = "";
					...
					$.each(landing_list, function (k, i) {
						if (i != undefined) {
							$("#single_select_" + key).append("<option value='" + i.id + "'>" + i.bannerName + "</option>");
						}
					});
			...
			$(".btn_view_code").click(function () {
				var obj_temp = JSON.parse($(this).attr("item_obj"));
				editing_item = obj_temp.id;
				$('#modal_view_code .modal-title').html("Edit " + item_name + " " + obj_temp.name);
				var random_banner = obj_temp.splayBanners[Math.floor(Math.random() * obj_temp.splayBanners.length)];
				var random_landing = random_banner.splayLandingPageList[Math.floor(Math.random() * random_banner.splayLandingPageList.length)];
				var string_code = "<div>";
				string_code += "<a class='simba_landing'>";
				string_code += "<img class='simba_banner' />";
				string_code += "</a>";
				string_code += "<script src='http://code.jquery.com/jquery-3.3.1.min.js' integrity='sha256-FgpCb/KJQlLNfOu91ta32o/NMZxltwRo8QtmkMRdAu8=' crossorigin='anonymous'></script>";
				string_code += "<script>";
				string_code += "$(document).ready(function () {";
				string_code += "$.ajax({";
				string_code += "url: '" + base_request_url + "/anonymous/splayBanner?campaignId=" + editing_item + "',";
				string_code += "type: 'GET',";
				string_code += "contentType: 'application/json',";
				string_code += "dataType: 'json',";
				string_code += "success: function (result) {";
				string_code += "$('.simba_landing').attr('href','http://dev.simba-app.com/anonymous/clickSplayBanner?campaignId=" + editing_item + "&bannerId='+result.dataObj.dataResponse.id+'&deviceId=1');";
				string_code += "$('.simba_landing').attr('target','_blank');";
				string_code += "$('.simba_banner').attr('src',result.dataObj.dataResponse.defaultImage);";
				string_code += "},";
				string_code += "error: function (error) {";
				string_code += "},";
				string_code += "});";
				string_code += "});";
				string_code += "</script>";
				string_code += "</div>";
				$('#modal_view_code .area_code').val(string_code);
				$('#modal_view_code').modal('show');
			});
		},
	});
	...
	$('.btn_confirm_yes').click(function () {
		$.ajax({
			url: base_request_url + "/admin/campaign?id=" + deleted_id,
			type: "DELETE",
			contentType: "application/json",
			dataType: "json",
			beforeSend: function (xhr) {
...
jQuery(document).ready(function () {
	...
	$('.btn_add_more_item').click(function () {
		...
		$.each(landing_list, function (key, item) {
			$("#single_select_" + item_count).append("<option value='" + item.id + "'>" + item.bannerName + "</option>");
		});
		$('.btn_delete_item').click(function () {
			$(this).parent().parent().remove();
		});
	...
	$('.btn_modal_copy').click(function () {
		...
		show_toast_notif("success", "Completed", "Success", function () { });
	});
	$.ajax({
		...
		success: function (data) {
			...
			$(data.dataArr).each(function (key, item) {
				...
				$("#single_select_0").append("<option value='" + item.id + "'>" + item.bannerName + "</option>");
			});
			load_autocomplete_component();
		},
		error: function (err) {
