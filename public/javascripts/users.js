		...
		columns: [
			...
			{ title: "Phone", data: "phoneNumber" },
			{
				...
				render: function (data, type, full, meta) {
					if (full.userGameRID != null) {
						var format = getSetting(_setting_data, "date_format");
						var news_date = moment.unix(full.userGameRID.createOn / 1000).format(format.value);
					} else {
						var news_date = "Unknown";
					}
					return news_date;
				}
			},
			{ title: "Role", data: "groupRole.name" },
			{
				title: "Status",
				data: null,
				render: function (data, type, full, meta) {
	...
	$('.btn_modal_save_update').click(function () {
		...
		$.ajax({
			...
			success: function (data) {
				...
				page_datatable.ajax.reload(null, false);
				if (data.onlyMessage == 'Success') {
					show_toast_notif("success", "Successfully", "Updated '" + $(".txt_add_edit_item_firstname").val() + " " + $(".txt_add_edit_item_lastname").val() + "'", function () {
						//action when click on notif
					});
				} else {
					show_toast_notif("error", "Fail", "Error", function () {
