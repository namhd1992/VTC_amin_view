...
var load_page = function () {
  ...
  page_datatable = $('#page_datatable').DataTable({
    ...
    "fnDrawCallback": function (oSettings) {
      ...
      $(".btn_edit_item").click(function () {
        ...
        $('#modal_add_edit .txt_add_edit_item_price').val(obj_temp.priceScoin);
        $('#modal_add_edit .txt_add_edit_item_gender').val(obj_temp.firstType);
        $('#modal_add_edit .txt_add_edit_item_category').val(obj_temp.secondType);
        $('.image_item').html('<img src=' + obj_temp.defaultImage + ' />');
        var format = getSetting(_setting_data, "date_format");
        $('.txt_add_edit_item_date_start').val(moment.unix(obj_temp.fromDate / 1000).format(format.value));
        $('.txt_add_edit_item_date_end').val(moment.unix(obj_temp.toDate / 1000).format(format.value));
        item_list_arr = obj_temp.itemOfPromotion;
  ...
  $('.btn_modal_save_new').click(function () {
    ...
    _send_info.data = {
      ...
      "defaultImage": $(".image_item img").attr("src"),
      "fromDate": new moment($(".txt_add_edit_item_date_start").val(), "YYYY-MMMM-D hh:mm").unix() * 1000,
      "toDate": new moment($(".txt_add_edit_item_date_end").val(), "YYYY-MMMM-D hh:mm").unix() * 1000,
      // "shopItemProList": item_list_arr,
    ...
    $.ajax({
      ...
      success: function (data) {
        if (data.statusCode == 'T') {
          ...
          $('#modal_add_edit .image_item').html("<img src='" + data.dataObj + "' />");
        } else {
