...
var item_name = "Spin purchase history";
var item_names = "Spin purchase history";
var deleted_id = null;
var channel_arr = [];
var page_datatable = null;
_current_page = 1;
var search_value_user = null;
var search_value_spin = null;
var start_time = null;
var end_time = null;
var sender = null;
...
var load_page = function() {
    ...
    page_datatable = $('#page_datatable').DataTable({
        "ajax": {
            ...
            "data": function(d) {
                ...
                d.filterBy.serviceType = "spent";
                d.filterBy.sourceType = "luckyspin";
        ...
        "columns": [
            { title: "ID", data: "id" },
            { title: "User", data: "sender.fullName" },
            { title: "Amount", data: "amount" },
            {
                ...
                render: function(data, type, full, meta) {
                    var return_str = "";
                    console.log(JSON.parse(full.dataRequest));
                    var data_json = JSON.parse(full.dataRequest);
                    if (data_json.spinName != undefined) {
                        return_str = data_json.spinName;
                    } else {
                        return_str = "Unknown";
                    }
                    return return_str;
            ...
            },
            {
                ...
                render: function(data, type, full, meta) {
                    ...
                    console.log(JSON.parse(full.dataRequest));
                    var data_json = JSON.parse(full.dataRequest);
                    if (data_json.numberTurns != undefined) {
                        return_str = data_json.numberTurns;
                    } else {
                        return_str = "Unknown";
                    }
                    return return_str;
    ...
    var load_autocomplete_component = function() {
        ...
        $(".select2, .select2-multiple, .select2-allow-clear, .js-data-example-ajax").on("select2:open", function() {
            ...
            if ($(this).attr("id") == "single_select_spin") {
                //event for search field
                typing_input = $('.select2-search__field')[0];
                $(typing_input).on("keypress", function() {
                    console.log("press");
                    ...
                    typingTimer = setTimeout(function() {
                        ...
                        $.ajax({
                            ...
                            success: function(data) {
                                ...
                                var tags_arr = [];
                                $(data.dataArr).each(function(key, item) {
                                    $("#single_select_spin").append("<option value='" + item.id + "'> " + item.name + "</option>");
                                });
                                load_autocomplete_component();
                                $("#single_select_spin").select2("open");
                                $($('.select2-search__field')[0]).val(searching_value_timeout);
                            },
                            error: function(err) {
                                console.log("error");
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (data.onlyMessage == 'Success') {
                    ...
                    show_toast_notif("error", "Fail", "Error duplicated giftcode");
                }
            },
            error: function(error) {
    ...
    $('.btn_modal_save_update').click(function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                page_datatable.ajax.reload(null, false);
                if (data.onlyMessage == 'Success') {
                    show_toast_notif("success", "Successfully", "Created '" + $(".txt_add_edit_item_firstname").val() + " " + $(".txt_add_edit_item_lastname").val() + "'");
                } else {
                    show_toast_notif("error", "Fail", "Error duplicated giftcode");
                }
            },
