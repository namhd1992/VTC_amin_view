...
var load_page = function() {
    ...
    page_datatable = $('#page_datatable').DataTable({
        "ajax": {
            ...
            "dataSrc": function(json) {
                ...
                $(json.dataArr).each(function(key, item) {
                    if (item.type == "income") {
                        input_total += parseInt(item.fundsAmount);
                        total_lp += parseInt(item.fundsAmount);
                    } else {
                        output_total += parseInt(item.fundsAmount);
                        total_lp += parseInt(item.fundsAmount);
                    }
                });
                report_data.push({
                ...
                var income_lp = [];
                income_lp.push({
                    y: parseFloat(((parseInt(input_total) / total_lp) * 100).toFixed(2)),
                    color: colors[color_order],
                    value: parseInt(input_total)
                });
                color_order++;
                income_lp.push({
                    y: parseFloat(((parseInt(output_total) / total_lp) * 100).toFixed(2)),
                    color: colors[color_order],
                    value: parseInt(output_total)
                });
                color_order++;
                var categories = ["Income", "Outcome"];
                var income_data = [];
                ...
                $(".widget_total_input").attr("data-value", json.dataObj.dataResponse.inputFundsAmount);
                $(".widget_total_output").attr("data-value", json.dataObj.dataResponse.outputFundsAmount);
                $(".widget_total_count").attr("data-value", json.totalRecords);
                $(".widget_total_input").counterUp();
                $(".widget_total_output").counterUp();
                $(".widget_total_count").counterUp();
                return json.dataArr;
            },
        ...
        "columns": [
            { title: "ID", data: "id" },
            { title: "Name", data: "fundsName" },
            { title: "Type", data: "type" },
            { title: "Amount", data: "fundsAmount" },
            {
                title: "Action",
                data: null,
                render: function(data, type, full, meta) {
                    var return_str = "";
                    if (full.type == "income") {
                        return_str += "<a class='btn_detail_link' item_id='" + full.id + "' href='/income_report?id=" + full.id + "'><i class='icon-calendar'></i> Details </a>";
                    } else {
                        return_str += "<a class='btn_detail_link' item_id='" + full.id + "' href='/outcome_report?id=" + full.id + "'><i class='icon-calendar'></i> Details </a>";
                    }
                    return return_str;
                }
            }
