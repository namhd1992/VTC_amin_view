...
var load_page = function() {
    ...
    if (localStorage.getItem("base_request_url") != null) {
        ...
        $('#single_select_env').val('http://localhost:8081');
    }
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            $(".loading_icon").hide();
            $(".txt_date_format").val(get_data_by_name(data.data, "date_format").value);
        },
        error: function(err) {
    ...
    $(".btn_submit").click(function() {
        $(".loading_icon").show();
        localStorage.setItem("base_request_url", $('#single_select_env').val());
        $(setting_data).each(function(key, item) {
            if (item.name == "date_format") {
                item.value = $(".txt_date_format").val();
            }
        });
        _send_info.data = {
            "dataArr": setting_data
        };
        $.ajax({
            url: base_request_url + "/admin/adbSetting",
            type: "PUT",
