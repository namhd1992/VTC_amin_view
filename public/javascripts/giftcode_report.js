...
var load_page = function () {
	...
	if (getUrlParameter("giftcodeId") !== undefined) {
		giftcodeId = getUrlParameter("giftcodeId");
	}
	page_datatable = $('#page_datatable').DataTable({
		"ajax": {
			"url": base_request_url + "/admin/timelineReportGiftcode",
			"type": "GET",
			"data": function (d) {
				d.limit = _per_page;
				d.offset = _per_page * (_current_page - 1);
				if(giftcodeId !== ""){
					d.giftcodeId = giftcodeId;
				}
			},
		...
		"columns": [
			...
			{ title: "From", data: "fromEmail" },
			{ title: "To", data: "toEmail" },
			{
				title: "Create On",
				data: null,
		...
		"fnDrawCallback": function (oSettings) {
			...
			$(".btn_edit_item").click(function () {
				// console.log($(this).attr("item_obj"));
	...
	$.ajax({
		...
		success: function (data) {
			...
			giftcodeId = data.dataArr[0].giftcodeEvent.id;
			page_datatable.ajax.reload(null, false);
			load_autocomplete_component();
		},
		error: function (err) {
